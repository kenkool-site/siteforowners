...
async function loadPhotos(previewSlug: string | null): Promise<{
  ...
  const { data } = await supabase
    .from("previews")
    .select("images, gallery_video_url, gallery_video_title")
    .eq("slug", previewSlug)
    .maybeSingle();
  ...
  const galleryVideoTitle =
    typeof data?.gallery_video_title === "string" && data.gallery_video_title.length > 0
      ...
      : null;
  return { images, galleryVideoUrl, galleryVideoTitle };
}
...
export default async function PhotosPage({
  ...
  if (!tenant) notFound();
  const { images, galleryVideoUrl, galleryVideoTitle } = await loadPhotos(tenant.preview_slug);
  return (
