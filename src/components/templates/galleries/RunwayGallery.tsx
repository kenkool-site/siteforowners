...
export function RunwayGallery({
  ...
  }, [activeMobileIndex, mobileSliderEnabled]);
  ...
  useEffect(() => {
    if (!mobileSliderEnabled) {
      if (resumeAutoTimeoutRef.current) {
        clearTimeout(resumeAutoTimeoutRef.current);
        resumeAutoTimeoutRef.current = null;
      }
      userPausedAutoRef.current = false;
      return;
  ...
  const scrollToMobileSlide = useCallback(
    ...
    [galleryImages.length, markUserControlsGallery, mobileSliderEnabled],
  );
  ...
  }, [galleryImages.length, mobileSliderEnabled, scrollToMobileSlide]);
