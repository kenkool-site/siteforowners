...
**Legacy events with a large un-backfilled backlog:** the "within 5 minutes" timeline above assumes descriptors already exist for the event's approved media. A legacy/pre-existing event can have zero descriptors the first time a host clicks Generate — `processHighlightGeneration` now waits for descriptor readiness rather than terminally failing (see the consolidated fix wave note below), attempting a bounded per-event catch-up (up to 20 items) on each claim attempt and re-queueing the same generation if any approved media is still undescribed afterward. Combined with the cron's own platform-wide backfill pass (5 items per 5-minute tick, shared across every event on the platform), a legacy event with N un-backfilled approved photos should realistically expect roughly `N / 20 * 5 minutes` from its own per-event catch-up alone, worst-cased down to `N / 5 * 5 minutes` if the platform-wide pool is the only thing making progress (e.g. many other legacy events are also catching up concurrently) — not a flat "within 5 minutes." The host does not need to do anything further after the first click: the generation stays queued and silently retries on each cron tick until backfill catches up, and re-triggering Generate manually at any time is harmless (it either reports the same pending generation or, once idle, queues a fresh one).
...
### Consolidated fix wave (2026-09-24)

The final whole-branch review found several real gaps between this spec and the shipped implementation, fixed in one consolidated pass after all eight tasks merged:

- Guest read-path eligibility (`getPublishedMemoryHighlights`) now keys off the published generation's own `mode`, not each group's `source` in isolation — fixes host-defined groups leaking into the guest AI Highlight tab permanently after a host round-trips through host-defined mode and back to automatic.
- `processHighlightGeneration` now waits for descriptor readiness (spec's Generation lifecycle step 3, previously unimplemented) instead of terminally failing a pre-existing event's first generation attempt with `EMPTY_HIGHLIGHT_OUTPUT` — see the legacy-event timeline note above.
- `generateHostDefinedAssignments` skips the Anthropic call entirely when zero host-defined groups exist yet, and the host UI now triggers a fresh generation the moment the first group is actually added.
- `highlight_generation_error` is now read back (`getHighlightGenerationState`/`getHostHighlightsOverview`) and shown to the host alongside the generic failure message.
- Descriptor eligibility (`listApprovedMemoryDescriptors`, and the `list_approved_media_missing_descriptors_across_events` RPC in migration 059) now also requires `processing_status = 'ready'`, matching `listGalleryVisibleMedia`'s guest-visibility filter.
- Dead code (`setAiClassifiedMoment`, `listMomentOverridesForEvent` — zero callers, and the former wrote to `memory_moment_media` in violation of this plan's Global Constraints) was deleted from `repository.ts`.

Several review findings were deliberately parked out of this fix wave as either too large for a single pass (automatic-group rename/reorder/hide UI) or low-risk as-is (generation-failure backoff, the orphaned-until-now `backfillMissingMemoryDescriptors`, generation-row pruning, response-envelope consistency, a stopword-matching risk) — see `.superpowers/sdd/2026-09-23-ai-highlight-grouping/progress.md`'s final review section for the complete list and rulings.
