...
- **Deliberate V1 simplification made in this plan, not stated verbatim in either spec:** Rekognition's Video moderation API is asynchronous (start/poll or SNS-notified) and materially more complex than the synchronous Image API. To avoid building that async job-polling infrastructure for V1, the *intent* was for video moderation to run Rekognition's **Image** moderation (`DetectModerationLabels`) against a client-captured poster frame — but **no poster-frame upload mechanism exists in this plan, its tasks, or either spec.** Corrected during final whole-branch review: video moderation is not "simplified" in Plan A, it is **absent** — the Worker's video path (Task 5) marks video rows `processing_status='ready'` with no moderation trigger at all, and `upload/init` rejects `mediaKind: "video"` outright (added to Task 4's route during the final-review fix wave) until a real video-moderation path — poster-frame upload plus this Image-moderation call, or equivalent — ships in a later plan. Do not assume video moderation "already works in a simplified form"; it does not exist yet.
